        ...
        <div class="row-fluid">
            <div class="span3">
                ...
                <ul id="accounts-menu" class="nav nav-list collapse in">
                  <li class="active"><a href="${pageContext.request.contextPath }/kehu/kehulist">客户</a></li>
...
<div class="btn-toolbar">
    <button class="btn btn-primary"><i class="icon-plus"></i><a href="${pageContext.request.contextPath }/manager/newkehu.jsp"> 新客户</a></button>
  <div class="btn-group">
  </div>
</div>
<div class="well">
    <table class="table">
      ...
      <tbody>
      <c:forEach items="${kehuList }" var="kehu" varStatus="status">
        <tr>
          <td>${status.count }<input type="hidden" value="${kehu.id }"/></td>
          <td>${kehu.khName }</td>
          <td>${kehu.khAddress }</td>
          <td>${kehu.khPhone }</td>
          <td>
              <a href="tomodify?id=${kehu.id }"><i class="icon-pencil"></i></a>
              <a href="#myModal" role="button" data-toggle="modal" onclick="tomyModal('${goods.id}')"><i class="icon-remove"></i></a>
          </td>
        </tr>
        </c:forEach>
      </tbody>
...
<div class="modal small hide fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-body">
        <p class="error-text"><i class="icon-warning-sign modal-icon"></i>确定要删除这一位客户吗?</p>
    </div>
    ...
    <script type="text/javascript">
    	...
    	function tomyModal(id){
    		...
    		$(".btn-danger").on("click",function(){
    			window.location.href = '${pageContext.request.contextPath }' + "/kehu/removeKeHu?id=" + id;
