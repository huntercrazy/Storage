        ...
        <div class="row-fluid">
            <div class="span3">
                ...
                <ul id="accounts-menu" class="nav nav-list collapse in">
                  ...
                  <li class="active"><a href="${pageContext.request.contextPath }/gongying/gongyinglist">供应商</a></li>
                </ul>
...
<div class="btn-toolbar">
    <button class="btn btn-primary"><i class="icon-plus"></i><a href="${pageContext.request.contextPath }/manager/newgongying.jsp"> 新供应商</a></button>
  <div class="btn-group">
  </div>
</div>
<div class="well">
    <table class="table">
      ...
      <tbody>
      <c:forEach items="${gongyingList }" var="gongying" varStatus="status">
        <tr>
          <td>${status.count }<input type="hidden" value="${gongying.id }"/></td>
          <td>${gongying.gyName }</td>
          <td>${gongying.gyAddress }</td>
          <td>${gongying.gyPhone }</td>
          <td>
              <a href="tomodify?gyName=${gongying.gyName }"><i class="icon-pencil"></i></a>
              <a href="#myModal" role="button" data-toggle="modal" onclick="tomyModal('${gongying.id}')"><i class="icon-remove"></i></a>
          </td>
        </tr>
        </c:forEach>
      </tbody>
...
<div class="modal small hide fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-body">
        <p class="error-text"><i class="icon-warning-sign modal-icon"></i>确定要删除这一家供应商吗?</p>
    </div>
    ...
    <script type="text/javascript">
    	...
    	function tomyModal(id){
    		...
    		$(".btn-danger").on("click",function(){
    			window.location.href = '${pageContext.request.contextPath }' + "/gongying/removeGongYing?id=" + id;
