...
<div class="btn-toolbar">
    <button class="btn btn-primary"><i class="icon-plus"></i><a href="${pageContext.request.contextPath }/manager/newgoods.jsp"> 新货物</a></button>
  <div class="btn-group">
  </div>
</div>
<div class="well">
    <table class="table">
      ...
      <c:forEach items="${goodsList }" var="goods" varStatus="status">
        <tr>
          <td>${status.count }<input type="hidden" value="${goods.id }"/></td>
          <td>${goods.goodsName }</td>
          <td>${goods.goodsType }</td>
          <td>${goods.goodsKind }</td>
          <td>${goods.content }</td>
          <td>
              <a href="tomodify?id=${goods.id }"><i class="icon-pencil"></i></a>
...
<div class="modal small hide fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-body">
        <p class="error-text"><i class="icon-warning-sign modal-icon"></i>确定要删除这一条商品数据吗?</p>
    </div>
    ...
    <script type="text/javascript">
    	...
    	function tomyModal(id){
    		...
    		$(".btn-danger").on("click",function(){
    			window.location.href = '${pageContext.request.contextPath }' + "/goods/removeGoods?id=" + id;
